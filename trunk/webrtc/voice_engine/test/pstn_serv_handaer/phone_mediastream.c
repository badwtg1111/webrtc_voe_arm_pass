...
void run_non_interactive_loop(MediastreamDatas* args) {
	...
	while(cond) //native global variable
	{
		int n;
		for(n=0;n<100;++n){
#ifdef WIN32
...
int mediastream_begin(MediastreamDatas* args){
	...
	if (args->eq){		
		if((pthread_create(&run_loop_tid, NULL, ( void * (*)(void *))&run_interactive_loop, (void *)args)) == -1){
		...
		#if 1
		printf("%s:%d -- %d\n", __FUNCTION__, __LINE__, (int)run_loop_tid);
		if((pthread_create(&run_loop_tid, NULL, ( void * (*)(void *))&run_non_interactive_loop, (void *)args)) == -1){
...
int mediastream_end(MediastreamDatas* args){
	...
	if ( run_loop_tid ){
		printf("%s:%d -- %d\n", __FUNCTION__, __LINE__, (int)run_loop_tid);
		pthread_join(run_loop_tid, NULL);
		printf("%s:%d -- %d\n", __FUNCTION__, __LINE__, (int)run_loop_tid);
	}
	clear_mediastreams(args);
